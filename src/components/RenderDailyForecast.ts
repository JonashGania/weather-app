...
import { DailyData } from "../interface/weatherData";

const DisplayDailyForecast = (dailyData: DailyData[]) => {
  const dailyForecastWrapper = <HTMLElement>document.querySelector(
  ...
  dailyData.slice(1).forEach((days) => {
    ...
    const dailyWeatherContainer = <HTMLDivElement>rowContainer.querySelector(
